...
export default function NewsCard({ article, index, categoryEmoji }) {
  ...
  const animationDelay = `${Math.min(index * 0.05, 0.6)}s`;

  return (
    <article
      className="news-card fade-in-up bg-[#161b22] rounded-2xl overflow-hidden border border-[#21262d] flex flex-col"
    ...
    >
      ...
      <div className="relative w-full h-[200px] overflow-hidden">
        {hasImage ? (
          ...
          <div className={`w-full h-full ${gradientClass} flex items-center justify-center`}>
            <span className="text-5xl opacity-60 select-none">{categoryEmoji}</span>
          </div>
        )}
      ...
      <div className="p-4 flex flex-col flex-1 gap-2">
        ...
        <a
          ...
          className="
            inline-flex items-center gap-1 mt-auto pt-3
            text-sm font-medium text-[#7c4dff] hover:text-[#b388ff]
